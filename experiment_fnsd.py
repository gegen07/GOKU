...
class Experiment():
    def __init__(self, args):
        ...
        print(np.unique(self.X_train[0].y.numpy()))
    ...
    def run(self):
        ...
        # for epoch in range(1, (self.max_epochs // self.eval_every) + 1):
            ...
            batch = copy.deepcopy(self.X_train[0])
            batch = Batch.from_data_list([batch])

            batch = batch.to(self.device)
            out, reff_per_layer = self.model(batch, self.pinvs, reff=True)
            
            # print(out[batch.train_mask].shape, batch.y[batch.train_mask].shape)
            loss = self.criterion(input=out[batch.train_mask], target=batch.y[batch.train_mask])
            total_num_examples += batch.y[batch.train_mask].size(0)
            total_loss += (loss.item() * batch.y[batch.train_mask].size(0))

            _, train_pred = out[batch.train_mask].max(dim=1)
            train_correct += train_pred.eq(batch.y[batch.train_mask]).sum().item()
            ...
            stopping_value = 0
            if self.stopping_criterion is STOP.VAL:
                if val_acc > best_val_acc + stopping_threshold:
                    best_val_acc = val_acc
                    best_train_acc = train_acc
                    best_epoch = epoch
                    epochs_no_improve = 0
                    stopping_value = val_acc
                    new_best_str = ' (new best val)'
                else:
                    epochs_no_improve += 1
            elif self.stopping_criterion is STOP.TEST:
                ...
                if train_acc > best_train_acc + stopping_threshold:
                    ...
                    epochs_no_improve += 1
            print(
                f'Epoch {epoch * self.eval_every}, LR: {cur_lr}: Train loss: {avg_training_loss:.7f}, Train acc: {train_acc:.4f}, Val acc: {val_acc:.4f}, Test accuracy: {test_acc:.4f}{new_best_str}, Effective Resistance: {reff_per_epoch_sum_layer}')
            ...
            if epochs_no_improve >= self.patience:
                ...
                break
            reff_arr.append((avg_training_loss, train_acc, test_acc, reff_per_epoch_sum_layer.cpu().numpy()))

        with open(f'reff_arr-{self.task}.pkl', 'wb') as f:
            pickle.dump(reff_arr, f)
    ...
    def eval_val(self):
        ...
        with torch.no_grad():
            total_correct = 0
            total_examples = 0
            batch = copy.deepcopy(self.X_test[0])
            batch = Batch.from_data_list([batch])
            # for batch in loader:
            batch = batch.to(self.device)

            pred = self.model(batch, None, reff=False)
            probabilities = F.softmax(pred[batch.val_mask], dim=1)  # Add softmax
            pred = probabilities.argmax(dim=1)

            total_correct += pred.eq(batch.y[batch.val_mask]).sum().item()
            total_examples += batch.y[batch.val_mask].size(0)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            pred = self.model(batch, None, reff=False)
            probabilities = F.softmax(pred[batch.test_mask], dim=1)  # Add softmax
            pred = probabilities.argmax(dim=1)

            total_correct += pred.eq(batch.y[batch.test_mask]).sum().item()
            total_examples += batch.y[batch.test_mask].size(0)
            acc = total_correct / total_examples
            return acc
