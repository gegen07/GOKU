from torch_geometric.nn import SGConv, SAGEConv, NNConv, GCNConv, GATConv, GPSConv, GraphSAGE, GINConv
...
def custom_forward(self, maps, edge_index, edge_attr=None):
    if self.gnn_layers > 0:
            ...
            if self.gnn_default:
                for module in self.phi:
                    maps = module(maps, edge_index)
        ...
        maps = maps.reshape(-1, self.d, self.hidden_channels).sum(dim=1)
        maps = self.phi(maps)
