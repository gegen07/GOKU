...
class GTREdgeBuilder:
    def __init__(self, data: torch_geometric.data.Data, try_gpu: bool = True):
        if try_gpu:
        ...
        self.laplacian = torch.tensor(
            nx.laplacian_matrix(self.graph).todense().astype("float")
        ).to(self.device)
    ...
    def compute_component_mask(self) -> torch.Tensor:
        """Return a n x n torch tensor where the i,j entry is 1 iff i and j are in the same connected component of `graph`"""

        def single_component_mask(vertices_of_component: list) -> torch.Tensor:
        ...
        component_mask = reduce(
            torch.logical_or,
            [
                single_component_mask(list(component))
                for component in nx.connected_components(self.graph)
            ],
        )
        return component_mask

    def update_edge_mask(self, s: int, t: int):
        """Update the edge mask after adding the edge {s,t}"""
        self.edge_mask[s, t] = 0
        self.edge_mask[t, s] = 0

    def update_laplacian(self, s: int, t: int):
        """Update the Laplacian after adding the edge {s,t}"""
        self.laplacian[s, s] += 1
        self.laplacian[s, t] -= 1
        self.laplacian[t, s] -= 1
        self.laplacian[t, t] += 1

    def update_pseudoinvere(self, s: int, t: int):
    ...
    def update_squared_pseudoinverse(self, s: int, t: int):
    ...
    def compute_edges(
        ...
        for _ in range(num_edges):
            ...
            resistance_matrix = (
                pinv_diagonal.unsqueeze(0) + pinv_diagonal.unsqueeze(1) - 2 * self.pinv
            )
            squared_pinv_diagonal = torch.diagonal(self.squared_pinv)
            biharmonic_matrix = (
                squared_pinv_diagonal.unsqueeze(0)
                + squared_pinv_diagonal.unsqueeze(1)
                - 2 * self.squared_pinv
            )
            ...
            s, t = divmod(
                torch.argmax(masked_diff_matrix).cpu().item(), self.data.num_nodes
            )
...
class AddGTREdges(BaseTransform):
    def __init__(self, num_edges: int, try_gpu: bool = True):
        print(
    ...
    def forward(self, data: torch_geometric.data.Data) -> torch_geometric.data.Data:
        if not hasattr(data, "precomputed_gtr_edges"):
            raise AttributeError(
                "Data does not have precomputed edges. Edges must be computed with the pre_transform PrecomputeGTREdges."
            )
...
class PrecomputeGTREdges(BaseTransform):
    ...
    def forward(self, data: torch_geometric.data.Data) -> torch_geometric.data.Data:
        data.precomputed_gtr_edges = GTREdgeBuilder(data, self.try_gpu).compute_edges(
            self.num_edges
        )
        return data
